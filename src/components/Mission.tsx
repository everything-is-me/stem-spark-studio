...
const CORE_VALUES: CoreValue[] = [
  ...
  },
  {
    ...
    delay: "0.1s",
    bgColor: "bg-gradient-to-br from-secondary/10 via-secondary/5 to-background"
  },
  {
    icon: Cpu,
    ...
    delay: "0.2s",
    bgColor: "bg-gradient-to-br from-accent/10 via-accent/5 to-background"
  },
  {
    icon: Smartphone,
...
interface ValueCardProps {
  value: CoreValue;
  onHover: (bgColor: string | null) => void;
}
...
const ValueCard: React.FC<ValueCardProps> = ({ value, onHover }) => {
  ...
  return (
    <div
      ...
      onMouseEnter={() => onHover(value.bgColor)}
...
const Mission = () => {
  ...
  return (
    <section className={`py-20 relative overflow-hidden transition-all duration-500 ${
      hoveredBg ? `${hoveredBg} bg-opacity-30` : 'bg-card'
    }`}>
      {/* Background pattern - only show when not hovering */}
      {!hoveredBg && (
      ...
      {/* Dynamic background overlay when hovering */}
      {hoveredBg && (
        <div className={`absolute inset-0 ${hoveredBg} opacity-20 transition-opacity duration-500`}></div>
      )}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {CORE_VALUES.map((value, index) => (
            <ValueCard key={index} value={value} onHover={setHoveredBg} />
          ))}
        </div>
